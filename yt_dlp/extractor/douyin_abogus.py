...
# ============================================================================
# SM3 Hash Implementation (from gmssl library - pure Python)
# ============================================================================
...
def _sm3_cf(v_i, b_i):
    ...
    for j in range(16, 68):
        w.append(0)
        w[j] = _sm3_p_1(w[j - 16] ^ w[j - 9] ^ _sm3_rotl(w[j - 3], 15)) ^ _sm3_rotl(w[j - 13], 7) ^ w[j - 6]
    ...
    for j in range(0, 64):
        ss_1 = _sm3_rotl(
            (_sm3_rotl(a, 12) + e + _sm3_rotl(SM3_T_J[j], j % 32)) & 0xffffffff, 7
        )
        ss_2 = ss_1 ^ _sm3_rotl(a, 12)
        tt_1 = (_sm3_ff_j(a, b, c, j) + d + ss_2 + w_1[j]) & 0xffffffff
        tt_2 = (_sm3_gg_j(e, f, g, j) + h + ss_1 + w[j]) & 0xffffffff
        d = c
        c = _sm3_rotl(b, 9)
        b = a
        a = tt_1
...
def _sm3_hash(msg):
    """SM3 hash function. msg should be a list of bytes (integers 0-255)."""
    len1 = len(msg)
    reserve1 = len1 % 64
    msg.append(0x80)
    reserve1 = reserve1 + 1
...
def generate_a_bogus(params, method="GET", platform="Win32"):
    """Generate a_bogus signature for Douyin API requests"""
    bogus = ABogus(platform)
    return bogus.get_value(params, method)


__all__ = ["ABogus", "generate_a_bogus"]
